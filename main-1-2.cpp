...
int main() {
  ...
  std::cout << "The orchestra currently has "
            << orchestra.get_current_number_of_members() << " members"
            << std::endl;
  ...
  for (int i = 0; i < orchestra.get_current_number_of_members(); i++) {
    std::cout << "Musician " << i + 1 << " plays "
              << members[i].get_instrument() << " and has "
              << members[i].get_experience() << " years of experience"
              << std::endl;
  }
  return 0;
}